...
//    let currentTarget: null
    ...
    public func preventDefault() {
        event.preventDefault.function?.callAsFunction(this: event.object)
    }
    
    /// Prevents other listeners of the same event from being called
    public func stopImmediatePropagation() {
        event.stopImmediatePropagation.function?.callAsFunction(this: event.object)
    }
    
    /// Prevents further propagation of an event during event flow
    public func stopPropagation() {
        event.stopPropagation.function?.callAsFunction(this: event.object)
    }
